...
export function WorkspaceOverlay({ taskId, isClient, isOpen, onClose }) {
  ...
  return (
    <div className="absolute inset-0 z-50 bg-[#F2F2F7] dark:bg-black animate-in slide-in-from-right-full duration-300 flex flex-col">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6 scrollbar-hide">
        ...
        <div className="bg-white dark:bg-[#1C1C1E] rounded-[28px] p-5 shadow-ios border border-black/[0.03] dark:border-white/[0.03] relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-4 relative z-10">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-[14px] font-black text-gray-900 dark:text-white tracking-tight">Umumiy progress</h3>
            </div>
            <span className="text-[16px] font-black text-[#007AFF]">{Math.round(progress)}%</span>
          </div>
          
          <div className="h-2.5 bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden relative z-10">
            <div 
              className="h-full bg-gradient-to-r from-[#007AFF] to-[#5856D6] transition-all duration-1000 ease-out"
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between px-1">
            <h3 className="text-[12px] font-black text-gray-400 uppercase tracking-[0.15em]">
              Loyiha bosqichlari
            </h3>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="bg-white dark:bg-[#1C1C1E] rounded-[32px] p-8 border border-dashed border-gray-200 dark:border-white/10 text-center space-y-6">
              ...
              <div className="space-y-2">
                <h4 className="text-[17px] font-black text-gray-900 dark:text-white">Ish maydoni bo'sh</h4>
              ...
              <div className="grid grid-cols-1 gap-3 pt-2">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3 px-4 py-2.5 bg-gray-50 dark:bg-white/5 rounded-2xl text-[12px] font-bold text-gray-600 dark:text-gray-400">
                    <span className="text-[#007AFF]">{tip.icon}</span>
                    {tip.text}
                  </div>
                ))}
              </div>
                ...
                <div 
                  ...
                  className={cn(
                    ...
                    m.isCompleted 
                      ...
                      : "bg-white dark:bg-[#1C1C1E] border-black/[0.02] dark:border-white/[0.03] shadow-sm"
                  )}
                >
                  <button 
          ...
          <div className="pt-4 pb-12">
            <form onSubmit={handleCreate} className="relative group">
              <input 
                ...
                className="w-full h-14 bg-white dark:bg-[#1C1C1E] border border-black/[0.03] dark:border-white/[0.03] rounded-2xl pl-5 pr-14 text-[14px] font-bold outline-none focus:ring-2 focus:ring-[#007AFF]/10 transition-all shadow-ios"
              />
              <button 
                type="submit" 
                disabled={!newTitle.trim() || create.isPending}
                className="absolute right-2 top-2 w-10 h-10 rounded-xl bg-[#007AFF] text-white flex items-center justify-center shadow-lg shadow-[#007AFF]/20 disabled:opacity-30 disabled:shadow-none transition-all active:scale-90"
              >
