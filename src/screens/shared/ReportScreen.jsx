...
export default function ReportScreen() {
  ...
  return (
    <PageLayout>
      <Header title="Shikoyat qilish" showBack onBack={() => navigate(-1)} />
      <div className="p-4 flex flex-col h-full overflow-y-auto pb-24 space-y-6">
        
        <div className="bg-edu-urgent/10 p-4 rounded-xl border border-edu-urgent/20">
          <p className="text-sm text-edu-urgent font-medium">
            ...
            {REPORT_TYPES.map(type => (
              <label key={type.value} className="flex items-center p-3 border border-edu-border/50 rounded-xl bg-edu-surface cursor-pointer active:scale-95 duration-[120ms]">
                <input 
                  ...
                  className="w-4 h-4 text-edu-urgent border-edu-border focus:ring-edu-urgent"
                />
                <span className="ml-3 text-sm font-bold text-edu-text">{type.label}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-edu-text mb-2 opacity-80">Batafsil ma'lumot</label>
          <textarea 
            value={formData.reason}
            onChange={(e) => setFormData({ ...formData, reason: e.target.value })}
            className="w-full bg-edu-surface border border-edu-border rounded-xl px-4 py-3 text-edu-text focus:outline-none focus:ring-2 focus:ring-edu-urgent min-h-[120px]"
            placeholder="Muammo haqida batafsil yozing..."
          />
        </div>

        <div className="mt-auto pt-4">
          <Button variant="danger" fullWidth onClick={handleSubmit} isLoading={loading}>
            Shikoyatni yuborish
          </Button>
        </div>
      </div>
