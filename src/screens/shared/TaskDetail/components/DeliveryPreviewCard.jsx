    ...
    <button 
      className={cn(
        ...
        isProtected 
          ...
          : "bg-edu-bg border-black/[0.03] dark:border-white/[0.05]"
      )}
      onClick={onView}
    >
      <div className={cn(
        "w-7 h-7 rounded-lg flex items-center justify-center shrink-0",
        isProtected ? "bg-amber-100 dark:bg-amber-500/10 text-amber-600" : "bg-edu-primary/10 text-edu-primary"
      )}>
        <FileText size={16} />
      </div>
      <span className="text-[13px] font-bold text-gray-700 dark:text-gray-300 truncate flex-1 text-left">{label}</span>
      <div className="text-[10px] font-bold text-edu-primary uppercase tracking-wider">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[14px] font-bold text-edu-text leading-tight tracking-tight">Vazifa natijasi</p>
          ...
          {isFullRevealed ? (
            <span className="text-[9px] font-bold bg-emerald-500/10 text-emerald-600 px-3 py-1.5 rounded-full border border-emerald-500/10 tracking-widest uppercase">
              Ochilgan
            </span>
          ) : (
            <span className="text-[9px] font-bold bg-amber-500/10 text-amber-600 px-3 py-1.5 rounded-full border border-amber-500/10 tracking-widest uppercase animate-pulse">
              Himoyalangan
            </span>
          )}
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            <p className="text-[10px] font-bold text-edu-muted uppercase tracking-[0.15em] px-1 flex items-center gap-2">
            ...
            {isClient && !isFullRevealed ? (
              <div className="relative rounded-2xl overflow-hidden">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-white/40 dark:bg-black/40 backdrop-blur-[2px] z-10">
                  <p className="text-[12px] font-bold text-edu-text text-center px-8 leading-relaxed">
                    {task.status === 'COMPLETED' 
                      ? 'Baho qoldiring va fayllarni oching' 
                      : 'Natijani tasdiqlang va fayllarni oching'}
                  </p>
                  {task.status === 'COMPLETED' && (
                    <Button size="sm" variant="primary" onClick={onLeaveReview} className="text-[11px] px-5 h-9 rounded-xl font-bold uppercase tracking-wider shadow-ios-primary">
                      Baho qoldirish ⭐
                    </Button>
                  )}
                </div>
              ...
              <div className="grid gap-2">
                ...
                {isFullRevealed && (
                  <div className="flex items-center justify-center gap-2 text-emerald-600 text-[11px] font-bold bg-emerald-500/5 py-3 rounded-xl border border-emerald-500/10">
                    <ShieldCheck size={14} />
                    Hujjatlar ochildi
                  </div>
                )}
        ...
        {(delivery.revisionCount || 0) > 0 && (
          <div className="flex items-center justify-center gap-1.5 text-[10px] text-gray-400 font-bold uppercase tracking-widest opacity-60">
            <AlertCircle size={12} />
