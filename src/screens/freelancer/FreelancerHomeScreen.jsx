...
function StatisticsWidget({ user }) {
  ...
  if (isLoading) return <div className="h-28 bg-edu-surface rounded-2xl animate-pulse mb-8" />;
      ...
      >
        <CardContent className="p-4 flex flex-col justify-between h-28">
          ...
          <div>
            ...
            <p className="text-[15px] font-bold text-edu-text mt-1 truncate">
      ...
      <Card className="bg-edu-surface border border-edu-border/20 relative overflow-hidden shadow-ios" radius="2xl">
        <CardContent className="p-4 flex flex-col justify-between h-28">
          ...
          <div>
            ...
            <p className="text-[17px] font-bold text-edu-text mt-1">{stats?.completedTasks || 0} ta</p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="bg-edu-surface border border-edu-border/20 relative overflow-hidden shadow-ios" radius="2xl">
        <CardContent className="p-4 flex flex-col justify-between h-28">
          ...
          <div>
            ...
            <p className="text-[17px] font-bold text-edu-text mt-1">
...
function ActiveTasksWidget() {
  const navigate = useNavigate();
  // We use useQuery directly here with throwOnError to let SectionErrorBoundary catch it
            ...
            {nearDeadlineTasks.map(task => (
              ...
              >
                <div className="min-w-0 pr-2">
                  <h4 className="text-xs font-bold text-red-900 dark:text-red-300 truncate">{task.title}</h4>
                  <p className="text-[10px] text-red-700 dark:text-red-400/80 mt-0.5">Topshirish muddati kam qoldi!</p>
                </div>
                <span className="text-[10px] bg-red-600 text-white font-bold px-2.5 py-1 rounded-xl shrink-0">
            ...
            >
              <CardContent className="p-3.5 space-y-2">
                <div className="flex justify-between items-start gap-2">
                  ...
                  <span className="text-[10px] font-bold text-edu-primary shrink-0">
...
function MyGigsWidget({ user }) {
  ...
  if (isLoading) return <div className="h-28 bg-edu-surface rounded-2xl animate-pulse" />;
            ...
            >
              <CardContent className="p-3 flex flex-col justify-between h-28">
                ...
                <div className="flex justify-between items-baseline pt-1 border-t border-edu-border/20">
                  ...
                  <span className="text-xs font-bold text-edu-primary">{formatPrice(gig.price)} so'm</span>
                </div>
              </CardContent>
            </Card>
          ))}
  ...
  return (
    <div className="flex flex-col h-full bg-edu-bg pb-24 p-4 overflow-y-auto scrollbar-hide">
      ...
      <div className="bg-gradient-to-br from-edu-accent to-[#7064E2] squircle p-6 text-white shadow-lg shadow-edu-accent/20 mb-8 relative overflow-hidden border border-white/20">
        ...
        <h2 className="text-2xl font-bold font-display mb-1 tracking-ios-display">Vazifalar qidirish</h2>
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 bg-white text-edu-accent px-6 py-3 rounded-[18px] font-bold text-[14px] shadow-md hover:bg-slate-50 transition-all active:scale-95 w-full md:w-auto"
        >
          <Search size={18} />
          Ishlarni ko'rish
        </button>
      ...
      {/* ── My Gigs ───────────────────────────────────── */}
      <SectionErrorBoundary fallbackTitle="Xizmatlarni yuklashda xatolik">
        <MyGigsWidget user={user} />
      </SectionErrorBoundary>
