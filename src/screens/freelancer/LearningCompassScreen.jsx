...
export default function LearningCompassScreen() {
  ...
  return (
    <PageLayout title="AI Learning Compass" showBack bgClass="bg-[#F2F2F7] dark:bg-black">
      <div className="p-4 space-y-6 pb-safe">
        ...
        <div className="bg-gradient-to-br from-edu-primary/10 to-transparent p-6 rounded-[28px] border border-edu-primary/20 shadow-sm relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h1 className="text-2xl font-black text-edu-text mb-2">Sizning AI Kompasingiz</h1>
        ...
        <div className="grid gap-4">
          ...
          <Card className="border-0 shadow-ios overflow-hidden">
            <div className="bg-red-500/10 p-3 flex items-center gap-2 border-b border-red-500/10">
              ...
              <span className="text-xs font-black text-red-600 uppercase tracking-wider">Zaif nuqtalar</span>
            </div>
          ...
          <Card className="border-0 shadow-ios overflow-hidden">
            <div className="bg-green-500/10 p-3 flex items-center gap-2 border-b border-green-500/10">
              ...
              <span className="text-xs font-black text-green-600 uppercase tracking-wider">Kuchli tomonlaringiz</span>
            </div>
            <CardContent className="p-4">
          ...
          <Card className="border-0 shadow-ios overflow-hidden">
            <div className="bg-edu-primary/10 p-3 flex items-center justify-between border-b border-edu-primary/10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-black text-edu-primary uppercase tracking-wider">Bozor talabi (Gaps)</span>
              </div>
                ...
                <div className="space-y-2">
                  {compass.demandGaps.map((gap, i) => (
                    <div key={i} className="flex items-center justify-between bg-edu-surface rounded-xl p-3 border border-edu-border/30">
                      ...
                      <span className={`text-[10px] font-black px-2 py-1 rounded-md ${gap.demandLevel === 'HIGH' ? 'bg-red-100 text-red-600' : 'bg-orange-100 text-orange-600'}`}>
          ...
          {compass.suggestedCourses?.length > 0 && (
            <div className="pt-4">
               <h3 className="text-xs font-black text-edu-muted uppercase tracking-wider mb-3 px-1 flex items-center gap-2">
                 <BookOpen className="w-4 h-4" /> 
                 Tavsiya etilgan kurslar
               </h3>
               <div className="space-y-3">
                 {compass.suggestedCourses.map((course, idx) => (
                   <div key={idx} className="bg-white dark:bg-[#1C1C1E] rounded-2xl p-4 shadow-sm border border-black/5 dark:border-white/5 active:scale-[0.98] transition-all cursor-pointer">
