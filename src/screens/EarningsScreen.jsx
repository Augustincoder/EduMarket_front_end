    ...
    <PageLayout showNav={true} bgClass="bg-edu-bg">
      ...
      <div className="pt-3 pb-24 px-4 h-full flex flex-col space-y-5 overflow-y-auto scrollbar-hide">
              ...
              {stats.weeklyEarnings.map((day, idx) => {
                ...
                return (
                  <div key={idx} className="flex flex-col items-center gap-1.5 flex-1">
                    ...
                    <div className="w-5 bg-slate-100 dark:bg-slate-800/80 rounded-lg h-16 relative overflow-hidden flex items-end">
                      <div 
                        className="w-full bg-gradient-to-t from-edu-primary to-indigo-500 rounded-lg transition-all duration-500" 
        ...
        {stats?.frozenEarned > 0 && (
          <div className="bg-indigo-50/50 dark:bg-indigo-950/20 border border-indigo-100/50 dark:border-indigo-900/30 rounded-2xl p-3.5 flex gap-2.5 items-start">
        ...
        <div className="flex bg-slate-100 dark:bg-slate-800/80 p-1 rounded-xl">
          <button 
            onClick={() => { hapticLight(); setPeriod('month'); }}
            className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${period === 'month' ? 'bg-white dark:bg-slate-700 shadow-sm text-edu-primary' : 'text-edu-muted'}`}
          >
            Bu oy
          </button>
          <button 
            onClick={() => { hapticLight(); setPeriod('3months'); }}
            className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${period === '3months' ? 'bg-white dark:bg-slate-700 shadow-sm text-edu-primary' : 'text-edu-muted'}`}
          >
            3 oy
          </button>
          <button 
            onClick={() => { hapticLight(); setPeriod('year'); }}
            className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${period === 'year' ? 'bg-white dark:bg-slate-700 shadow-sm text-edu-primary' : 'text-edu-muted'}`}
          >
            Shu yil
          </button>
        </div>
            ...
            <div className="space-y-2.5">
              {filteredCompleted.map((task) => (
                <Card 
                  key={task.id}
                  isPressable
                  onPress={() => { hapticLight(); navigate(`/tasks/${task.id}`); }}
