...
export function DialogContent({ children, className = '' }) {
  return (
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Content className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-lg z-50 focus:outline-none max-h-[90vh] overflow-y-auto ${className}`}>
        {children}
  ...
  return (
    <SelectPrimitive.Root value={value} onValueChange={onValueChange}>
      ...
      <SelectPrimitive.Portal>
        <SelectPrimitive.Content className="overflow-hidden bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 z-50">
