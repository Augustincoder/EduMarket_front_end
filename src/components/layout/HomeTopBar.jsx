        ...
        <div className="relative shrink-0 active-spring cursor-pointer" onClick={handleOpen}>
          <div className="relative">
            ...
            {unreadNotifications > 0 && (
              <span className="absolute -top-1 -right-1 bg-edu-urgent text-white text-[10px] font-black min-w-[20px] h-[20px] px-1 rounded-full flex items-center justify-center border-2 border-edu-surface shadow-sm animate-pulse">
                {unreadNotifications}
              </span>
            )}
          </div>
        ...
        <div className="flex flex-col gap-3 py-2 px-1">
          ...
          <div className="flex items-center gap-4 bg-edu-surface p-4 rounded-[20px] border border-edu-border/50 shadow-sm mb-2">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-sm font-black text-edu-text truncate">{user?.fullname}</h3>
              <p className="text-[11px] text-edu-muted font-bold truncate">@{user?.username}</p>
            </div>
            {user?.isVip && (
              <span className="bg-edu-vip/10 text-amber-500 text-[10px] font-black px-2 py-1 rounded-lg border border-edu-vip/20">
                VIP
              </span>
            )}
          </div>
          ...
          >
            <div className="w-12 h-12 rounded-2xl bg-edu-primary/10 text-edu-primary flex items-center justify-center relative shrink-0">
              <Bell size={24} />
              {unreadNotifications > 0 && (
                <span className="absolute -top-1 -right-1 bg-edu-urgent text-white text-[10px] font-black min-w-[18px] h-[18px] rounded-full flex items-center justify-center border-2 border-edu-surface">
                  {unreadNotifications}
                </span>
              )}
            </div>
            <div className="flex-1 text-left min-w-0">
              <h4 className="text-[15px] font-black text-edu-text mb-0.5 truncate">Bildirishnomalar</h4>
          ...
          >
            ...
            <div className="flex-1 text-left min-w-0">
              <h4 className="text-[15px] font-black text-edu-text mb-0.5 truncate">Mening Profilim</h4>
